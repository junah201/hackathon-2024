...
from app.utils.professor_oauth2 import get_current_professor

from typing import List, Optional

router = APIRouter(
    prefix="/professor_schedule",
)
...
def get_my_schedule(
    ...
    db_schedules: List[models.Schedule] = db.query(models.Schedule)\
        .filter(models.Schedule.professor_id == professor.id)\
...
def get_my_active_schedule(
    ...
    skip: int = 0,
    db: Session = Depends(get_db),
    user: models.Professor = Depends(get_current_professor)
):
    now = datetime.now()
