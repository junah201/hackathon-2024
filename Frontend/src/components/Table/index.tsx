...
export const Table = <ItemType extends any>({
  size = 20,
  title = '',
  showTotal = false,
  queryKey,
  ...
  return (
    <>
      ...
      <MainCard sx={{ p: 0 }}>
        ...
        {showPagenation && (
          <Pagination page={page} setPage={setPage} size={size} total={total} />
        )}
      </MainCard>
    </>
  );
...
const Pagination = ({ page, setPage, size, total }: PaginationProps) => {
  const MaxPage = Math.floor(total / size) + (total % size > 0 ? 1 : 0);
